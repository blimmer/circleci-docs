...
+
If rerun with SSH is a required feature, the SSH port (54782) must be opened.  The rule can be limited it scope by using the `--source-ranges` or `--source-tags` parameters.  The source must include the machine a user will SSH from.  The firewall rule may look like the following:
[source,shell]
----
